...
class AudioStreamerFragement extends Component {
    ...
    componentWillMount() {
        ...
        var filename = songs[songIndex].link;
        this._reloadPlayer(filename);
    }

    _updateState(err) {
        this.setState({
            pause: this.player.isPaused,
            duration: this.millisToMinutesAndSeconds(this.player.duration),
        });
    }
    ...
    _playPause() {
        console.log('this.player.canPlay', this.player.canPlay);
        console.log('this.player.isPlaying', this.player.isPlaying);
    ...
    onPressButtonNext = () => {
        ...
        this._reloadPlayer(songs[songIndex].link);
    ...
    onChangeSeekBarHandle = (ChangedValue) => {
        var seconds = ChangedValue * this.player.duration;
            ...
            <View style={{ flex: 1, flexDirection: 'column', backgroundColor: '#4e1676' }}>
                ...
                <View style={{ flex: 20, flexDirection: 'column', justifyContent: 'center' }}>
                    <View style={{ flex: 3, flexDirection: 'row', flexWrap: 'wrap', justifyContent: 'center', alignItems: 'center', marginLeft: 5, marginRight: 5 }}>
                        <CountTime player={this.player} pause={pause} isLoading={isLoading} />
                        <SeekBar player={this.player} pause={pause} isLoading={isLoading} onValueChange={this.onChangeSeekBarHandle} />
                    ...
                    <View style={{ flex: 7, flexDirection: 'row', flexWrap: 'wrap', justifyContent: 'center', alignItems: 'center', alignSelf: 'center' }}>
                        ...
                        <TouchableOpacity style={styles.buttonControl} onPress={() => this._playPause()}>
...
export default AudioStreamerFragement;